...
├── global.R                       Shared data loading (CSV reads only)
├── update_pipeline.R              Daily fetch script (run by GitHub Actions)
├── fetch_usbr_storage.R           USBR Hydromet fetch function
├── nohrsc_yakima_incremental.R    NOHRSC SWE incremental scraper
├── modules/
│   ├── mod_storage.R              Storage Dashboard tab
│   └── mod_explorer.R             Climate Explorer tab
├── www/
│   ├── styles.css
│   └── yakima_basin_map.png       Add your basin map image here
├── data/                          Pre-computed CSVs (refreshed daily)
│   ├── yakima_dam_daily.csv
│   ├── yakima_swe_combined.csv
│   └── ncei_climate_monthly.csv
└── .github/
    └── workflows/
        └── update_data.yml        GitHub Actions — 05:42 UTC daily
```
